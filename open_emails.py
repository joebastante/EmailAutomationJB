...
def generate_email_body(client, first_name, email_type, context):
    ...
    prompt = f"""I'd like you to craft an email to check in a professional connection and colleague. The email should be about 4 to 5 sentences in total to greet them, ask how the are doing, and check in on them. I want them to know I hope they are well I'm here to support them if ever needed. {context}. After you create the email, check it for grammar and flow. Do not take my text verbatim but adjust to make the email hang together. The email should be started with "Hi {first_name}." on its own line followed by a blank line and then the email text The tone should be casual but professional. Wording should be natural and sincere. This person has a relationship value of "{email_type}". Use the relationship types below to adjust the tone based on the depth of relationship I have with each person.
